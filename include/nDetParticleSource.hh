...
class nDetParticleSource : public G4GeneralParticleSource {
  public:
	...
	/** Enable or disable true isotropic source mode
	  ...
	  */
	void SetRealIsotropicMode(bool state_=true){ realIsotropic = state_; }
	...
	void SetAlphaBeam(const double &energy_);
	  ...
	  * @param energy_ Energy of the beam (in MeV)
	  */	
	void SetIonBeam(const double &energy_, const int &A, const int &Z, const double &exE=0);
  ...
  private:
	...
	bool realIsotropic; ///< Flag indicating that the source is truly isotropic (4pi) and not psuedo-isotropic
	...
	/** Sample a point on the surface of the unit sphere
	  ...
	  */
	static void unitSphereRandom(double &theta, double &phi);

	/** Convert spherical coordinates (with unit radius) to cartesian coordinates
	  * @param theta Polar angle
	  * @param phi Azimuthal angle
	  * @return A G4ThreeVector with contents (x, y, z)
	  */	
	static G4ThreeVector sphere2cart(const double &theta, const double &phi);
};
